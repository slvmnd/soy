...
type escaper struct {
	...
	printNodeEdits map[*ast.PrintNode][]string // TODO: is it valid to have multiple values?
}
...
// we do not edit text nodes.
func (e *escaper) escapeText(c context, n *ast.RawTextNode) context {
	s, i := n.Text, 0
	for i != len(s) {
		c1, nread := contextAfterText(c, s[i:])
		i1 := i + nread
		if c.state == statePCDATA || c.state == stateRCDATA {
			end := i1
			if c1.state != c.state {
				for j := end - 1; j >= i; j-- {
					if s[j] == '<' {
